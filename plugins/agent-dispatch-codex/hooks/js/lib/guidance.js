...
function mainAgentGuidance(config, compact = false) {
  ...
  if (compact) {
    const lines = [
      ...
      '默认不审查或格式化/lint 第三方实现（3rd、thirdparty、third_party、thridpart、vendor、external、deps、packages 等目录），除非用户明确指定；只核对自有代码的集成与调用契约，必要时读取依赖接口。',
  ...
  const lines = [
    ...
    '- Exclude vendored third-party implementations (3rd, thirdparty, third_party, thridpart, vendor, external, deps, packages and equivalent directory names) from review, formatting, and lint unless explicitly requested. Review first-party integration contracts and read only necessary dependency interfaces for context.',
