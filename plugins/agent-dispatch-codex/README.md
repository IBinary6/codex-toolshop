...
默认审查排除 `3rd`、`third_party`、`third-party`、`thridpart`、`vendor` 等第三方实现目录，只核对业务接入和调用契约，按需读取依赖接口；用户明确要求时才扩大到依赖实现审查。不得顺带格式化或 lint 第三方源码。依赖敏感的 include 顺序及其局部 clang-format 保护须保留，是否阻塞仍以实际影响证据为准。
