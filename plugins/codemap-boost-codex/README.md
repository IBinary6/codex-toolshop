...
MCP 启动器定位脚本后，会切换到插件数据目录运行，子进程也使用该目录，避免 Windows 因常驻进程的工作目录位于插件缓存而阻止更新备份。数据目录不代表目标项目，图查询仍须传入真实 `repo_root`，Serena 仍须激活目标项目。升级前已启动的旧进程不会自动改变工作目录；若仍报缓存占用，应在当前任务完成后正常退出并重新打开 Codex。
