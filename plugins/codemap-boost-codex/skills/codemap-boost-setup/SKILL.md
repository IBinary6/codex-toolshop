...
After capability discovery confirms absence, use available source inspection and check whether the task predates installation. Known files do not require a graph query. For ordinary repository text search, prefer the ready `tgrep-search-codex` wrapper, whose own hook supplies the absolute command (`node <plugin>/scripts/tgrep.cjs search [--fresh] <args>`). When that wrapper is missing, its index is incomplete or unhealthy, the scope does not fit, or immediate content is required, use a live `rg` scan or `tgrep --no-index`. A zero result is not evidence that content does not exist. Check final completeness and files just changed with a live scan. Use `tgrep --files` for healthy indexed discovery and `rg --files` for a live file list. Higher-level host rules take precedence.
...
- Preserve graph-first reminders at session, structural-prompt and subagent boundaries. For structural questions, query available graph tools first and then verify source. Known-file reads remain direct; ordinary repository text search follows the `tgrep-search-codex`/live-scan routing above. CodeMap only supplies graph routing and freshness: it neither installs nor starts tgrep, and it does not add another tgrep reminder hook.
