...
const GUIDANCE = [
  '1. 代码定位与协作：查找实现、函数、类、调用关系、依赖和影响面时，先用 code-review-graph 定位，再按返回的路径和行号读取源码；需要精确的符号定义、实现或引用时，用可用的 Serena/LSP 补充。图未命中、不可用或覆盖不足时，可用语义工具继续定位，或用已安装且就绪的 tgrep-search-codex 查文本，否则使用 rg 等实时搜索。候选符号或路径可按需交给图或 Serena 继续查证；证据充分即停止，无需轮流调用所有工具。',
  '2. 范围与刷新：图能力仅适用于 Git 工作树，每个 worktree 使用独立根目录和索引；hooks 负责刷新及读取前 barrier，主代理和子代理直接查询。只有 hook 报告失败、用户要求重建或进行 setup/诊断时，才手动更新。',
  '3. 查询与证据：问题明确时直接选用对应工具，需要图概览时查一次；信息不足就扩大查询或读取源码。工具未显示先检查延迟加载与工具发现能力。使用 Serena 前确认并激活当前目标项目的绝对根目录，切换项目后重新核对；插件目录不是目标项目。已知文件直接读；纯文本、日志、配置键或文件名枚举可直接使用文本工具。刚修改的内容和最终完整性核查使用实时源码读取或扫描。图或语言服务不可用、覆盖不足时说明限制；文本命中不等于关系，零命中不证明不存在。',
].join('\n');
