...
const GUIDANCE = [
  '1. 工具与目标：结构定位时，在当前工具目录与延迟发现能力中分别查找 code-review-graph 与 Serena。调用图工具时显式传入真实目标 Git 工作树的 repo_root；使用 Serena 前激活并核对真实目标项目的绝对路径。会话 cwd 和插件目录都不代表目标项目。',
  '2. 文本与回退：已知文件直接读；纯文本、日志、配置键或文件名枚举先按 tgrep-search-codex 的 CLI 入口核验可用性，并以目标目录为 workdir；未就绪则使用 rg 等实时搜索。任一工具缺失只表示当前时点不可用：立即回退，在下一轮或自然需要时再发现，不忙等也不记录永久不可用。',
  '3. 证据与刷新：按证据类型选择并互补：图谱查关系和影响面，Serena/LSP 查精确符号，tgrep/rg 查文本；证据充分即停止，不强制每次跑完三套。图谱仅适用于 Git 工作树，每个 worktree 独立；hooks 负责刷新及读取前 barrier，失败或 setup/诊断时才手动更新。文本命中不等于关系，零命中不证明不存在。',
].join('\n');
