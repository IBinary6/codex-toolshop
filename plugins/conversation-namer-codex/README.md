...
默认从实时 `model/list` 的可见文本模型中定位低成本候选，不写死具体型号或版本。程序优先使用模型描述中的明确低成本信号；描述没有低成本信号时，才以 Luna、Mini、Nano 等版本无关的轻量家族名称兜底。`model/list` 当前不提供可比较的实际价格，因此这里是低成本定位，不声称按真实价格排序。
