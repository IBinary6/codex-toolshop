...
默认使用 `gpt-6-luna`，并从实时 `model/list` 核对它是否可用；不可用时保留原标题，不自动改用旧型号。显式配置 `model: "auto"` 时，按模型描述中的低成本信号选择可用文本模型，描述没有低成本信号时以 Luna、Mini、Nano 家族兜底。目录不提供可比较的实际价格，因此自动选择不声称按真实价格排序。

候选必须明确支持 `none`、`minimal`、`low` 之一，生成时使用该模型实际声明的最低档位，不会伪造未支持的 effort。同一定位层级优先最低支持 effort，其余并列项保留目录顺序。隐藏模型、非文本模型和只有更高 effort 的模型不会参与自动选择；没有合适候选时保留原标题，不会默认改用大型模型。命名子进程固定使用 `default` 服务档，不继承主任务的 Fast 或 priority 档位。

可在插件 `PLUGIN_DATA/config.json` 中指定模型或超时，缺省无需配置：

```json
{ "model": "gpt-6-luna", "timeoutSeconds": 60 }
```

`model` 可改为当前账号可用的完整模型 ID；`timeoutSeconds` 支持 5–120 秒。配置只影响命名，不修改 Codex 全局模型。复用现有登录与 provider 配置，每个新会话至多发起一次命名推理。

命名执行程序优先跟随当前宿主：Windows/macOS 识别到桌面任务时使用桌面版自带的 Codex，识别到 CLI 任务时使用 CLI。未明确标识宿主时自动查找 PATH 中的 CLI 和桌面版；其他平台未识别的桌面布局保留 PATH 启动方式。不固定用户名或版本目录，也不会因为模型调用失败而切换宿主。
