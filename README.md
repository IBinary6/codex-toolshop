...
| `agent-dispatch-codex` | 面向产品、设计、QA、研究、运营和开发，按调查、规划、制作、验证、审查分配有界任务与模型；代码任务按取证、实现、验证和审查分阶段处理，低成本子任务默认使用配置候选。 | 新会话自动注入调度策略；按交付物验收，子代理直接执行、报告结果，并在整合后及时释放。 |
...
- 默认由 Sol medium 负责代码实现及测试代码修改；Luna max 处理探索、日志、常规文档/结构化数据、文本提取、代码取证和既定测试执行。低成本劳动力路线与代码写作分开，低成本路线不可用时不静默升级模型。
...
- 非琐碎交付经过针对性验证后独立审查，默认提供 Astra/xhigh 和高风险 Astra/ultra 候选；每次新建或复用审查代理，按实际职责、歧义、风险、质量和总成本自主选择有效候选，用户明确指定优先，职责或风险变化时重新评估。小范围复查也需要核对模型与档位；纯取证、既定测试与审查结论分开，核实实质问题后有界修复、重跑受影响检查并复查。小改和用户指定的主代理限定保持相称流程。
- 验证对应交付物：产品核对需求覆盖，设计核对体验与一致性，QA 按验收计划复现，研究核对来源，数据核对口径，代码执行相关测试。审查只把证实影响当前验收目标的缺陷作为返修依据；代码场景继续核对构建配置与调用路径，并保留没有缺陷证据的用户调试分支。
- 角色配置是候选默认值，关键词不构成授权或强制分派。只覆盖模型时使用插件的适用推理默认值，不把父任务的 ultra 自动带入另一模型；显式模型与档位保留，实际可用组合由宿主核实。接收审查结论前依据宿主运行元数据核对模型，无法观察时标记未确认；当前 Hook 传达核对要求，不提供实际 spawn 模型的硬拦截，详见[每次审查的模型核对](plugins/agent-dispatch-codex/README.md#每次审查的模型核对)。
- 路由提取只读、主代理限定、代理数量和已有计划的线索，并标明启发式候选身份。产品行为和引用材料不等于任务限制，旧路线也不形成跨轮约束；主代理按完整对话和最新明确要求推进已授权目标，保留真实用户约束。
